...
int playSmithy(int handPos, int currentPlayer, struct gameState *state) {
	...
	int i;
	for (i = 0; i < 4; i++) {  // Bug placed here. 4 instead of 3.
...
int playAdventurer(int temphand[], int z, int drawntreasure, int cardDrawn, int currentPlayer, struct gameState *state) {
...
int playCutpurse(int handPos, int currentPlayer, struct gameState *state) {
	int i, j, k;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	{
	case adventurer:
		playAdventurer(temphand, z, drawntreasure, cardDrawn, currentPlayer, state);
		//     while(drawntreasure<2) {
		// if (state->deckCount[currentPlayer] <1) {//if the deck is empty we need to shuffle discard and add to deck
		//   shuffle(currentPlayer, state);
		// }
